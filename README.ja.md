...
## Current Scope (v1.2.5)

現在の `v1.2.x` ラインは、ローカル、プライベートネットワーク、セルフホストでの実運用を対象にしています。Docker デプロイ、内蔵ログイン、HTML インポート、メタデータ永続化、フィルター、読書、アーカイブ、公開共有、ナレッジベース Q&A、V2 マルチエージェント HTML 生成 workflow を含みます。
...
- ナレッジベース Q&A は安定して提供済みの workflow です。HTML 生成、修正、ナレッジ管理 workflow は beta 段階であり、マルチエージェント routing、material handling、visual quality check を継続的に改善しています。
...
静的モードは読み取り専用公開、GitHub Pages のようなホスティング、生成アプリの確認に適しています。実際のアップロードとメタデータ永続化には、バックエンド API または既定の Docker デプロイが必要です。公開 demo は live workspace と同じ見た目に寄せていますが、backend と AI 操作は静的 preview として表示するだけで、実処理には接続しません。
