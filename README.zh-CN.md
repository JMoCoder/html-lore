...
当前 `v1.2.x` 版本线面向本地、私有网络和自托管的真实使用：Docker 部署、内置登录、HTML 导入、元数据持久化、筛选、阅读、归档、公开分享、知识库问答，以及 V2 多智能体 HTML 生成工作流。
...
- 知识库问答是已稳定交付的工作流。HTML 生成、修改和知识库管理仍处于 beta 阶段，多智能体路由、材料处理和视觉质量检查仍在持续演进。
...
静态模式适合只读发布、类似 GitHub Pages 的托管，或检查生成后的应用。真实上传和元数据持久化需要后端 API 或默认 Docker 部署。公开 demo 会保持与正式工作台一致的界面风格，但后端与 AI 操作只作为静态预览展示，不接入真实处理链路。
...
公开分享默认使用**安全静态分享**。服务端会先扫描原文件；通过检查的笔记直接生成链接。若文件包含脚本、交互控件、图表、不安全资源或本地引用等可修复问题，HTMlore 会在原文件同级生成静态安全副本，并分享该副本。副本会记录与原笔记的关联，且不会被发送给 AI 服务商。

对于用户可控内容，可明确选择**互动分享**以保留脚本和远程资源。内容仍只会在带 `allow-scripts` 的 iframe 沙箱中运行，不具备同源访问、表单、弹窗或顶层导航权限。嵌入的密钥、`<base>`、刷新跳转、危险 URL 协议、危险下载、`object` 和 `embed` 仍会被硬拦截；私有或本地引用需要二次确认。

部署时设置 `HTML_LORE_SHARE_INTERACTIVE_ENABLED=false` 可以关闭互动分享选项，例如面向托管或多租户环境时；安全静态分享仍然可用。
